...
def strat(data):
    ...
    results = linear_regression_model(data, features, target, regularization="ridge")
    ans = data[
        [
            "open_btc",
            "high_btc",
    ...
    t = 0.00054
    t2 = 0
    with open(path_from_relative_path("final_eth_1h.txt"), "a") as file:
    ...
    ans["datetime"] = data.loc[ans.index, "datetime"]
    ...
    for i in range(len(data)):
        ...
        lat_var = data["hidden_state"].iloc[i].astype(int)
...
def linear_regression_model(data, features, target, regularization="ridge", alpha=1.0):
    df_train = data.copy()
    df_test = data.copy()
